...
elasticsearch.prototype.get = function (limit, offset, total, callback) {
  var self = this
  ...
  self.version('input', function (err) {
    ...
    if (type === 'data') {
      self.getData(limit, offset, total, callback)
    } else if (type === 'mapping') {
      self.getMapping(limit, offset, total, callback)
    } else if (type === 'analyzer') {
      self.getSettings(limit, offset, total, callback)
    } else {
...
elasticsearch.prototype.getMapping = function (limit, offset, total, callback) {
  var self = this
...
elasticsearch.prototype.getSettings = function (limit, offset, total, callback) {
  var self = this
...
elasticsearch.prototype.getData = function (limit, offset, total, callback) {
  var searchRequest, self, uri
  self = this
  var searchBody = self.searchBody

  if ((offset >= self.totalSearchResults && self.totalSearchResults !== 0) || (total != 0 && offset >= total)) {
    callback(null, [])
    return
  }
...
// return (error, writes)
elasticsearch.prototype.set = function (data, limit, offset, total, callback) {
  var self = this
  var type = self.parent.options.type
  self.version('output', function (err) {
    ...
    if (type === 'data') {
      self.setData(data, limit, offset, total, callback)
    } else if (type === 'mapping') {
      self.setMapping(data, limit, offset, total, callback)
    } else if (type === 'analyzer') {
      self.setAnalyzer(data, limit, offset, total, callback)
    } else {
      callback(new Error('unknown type option'), null)
    }
  })
...
elasticsearch.prototype.setMapping = function (data, limit, offset, total, callback) {
  var self = this
...
elasticsearch.prototype.setAnalyzer = function (data, limit, offset, total, callback) {
  var self = this
...
elasticsearch.prototype.setData = function (data, limit, offset, total, callback) {
      ...
      if (r.items !== null && r.items !== undefined) {
        if (r.ok === true) {
          ...
          r.items.forEach(function (item) {
            if (item['index'].status < 400) { writes++ }
          })
        }
      }
